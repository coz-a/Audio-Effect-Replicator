...
def log_spectral_distance(
    ...
    Both signals are first rounded to the 16-bit PCM grid, so a prediction scores the
    same whether it is passed in directly or read back from the WAV it was saved to
    (otherwise near-silent bins dominate the log difference).
    """
    _check(pred, target)
